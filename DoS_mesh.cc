 ...
 *
 * Original Code Author: Kirill Andreev <andreev@iitp.ru>
 *
 *
...
#include "ns3/core-module.h" // O core module invoca os módulos de tracing, objeto e atributos todos juntos
...
/*****************************************************************************************************************************************/
/****************************** Essa primeira parte só foi definido os counters de Rx e Tx ***********************************************/
/*****************************************************************************************************************************************/
...
uint32_t g_udpTxCountMal = 0; //< Tx packet counter for malicious nodes.
uint32_t g_udpRxCountMal = 0; //< Rx packet counter for malicious nodes.
...
/*****************************************************************************************************************************************/
/************** Aqui a classe MeshTest é definida, primeiro com suas funções públicas, depois atributos e funções privadas ***************/
/*****************************************************************************************************************************************/
...
/*****************************************************************************************************************************************/
/***************************** Parece ser uma instância da classe, mas não tenho certeza *************************************************/
/***************************** Penso isso pq existem valores ao lado dos atributos privados **********************************************/
/*****************************************************************************************************************************************/
...
MeshTest::CreateNodes()
{
    ...
    /*
     * MAC address do mesh point root do protocolo HWMP
     * Checa se é um endereço broadcast
     * O padrão para esse endereço é um endereço broadcast: m_root("ff:ff:ff:ff:ff:ff")
     *
     * O m_stack é o protocolo a ser instalado. 
     * O padrão é o ns3::Dot11sStack -> Helper para instalar com facilidade 802.11s stack
     */
    ...
    /*
     * m_chan é uma variável que define se serão usados canais de frequência diferente para diferentes interface
     */
    if (m_chan)
    {
    ...
    // Set number of interfaces for each mesh point - default is single-interface mesh point
...
MeshTest::InstallApplication()
{
    ...
    // O m_packetInterval é o intervalo de envio dos pacotes em segundos
    // O padrão é 1 segundo de intervalo
    echoClient.SetAttribute("MaxPackets",
                            UintegerValue((uint32_t)(m_totalTime * (1 / m_packetInterval ))));
    echoClient.SetAttribute("Interval", TimeValue(Seconds(m_packetInterval)));
    echoClient.SetAttribute("PacketSize", UintegerValue(m_packetSize));
    // Define o nó de origem dos pacotes
    ...
    // Envio malicioso dos pacotes
    UdpEchoClientHelper echoClientMal(interfaces.GetAddress(sinkNodeId), portNumber);
    echoClientMal.SetAttribute("MaxPackets", UintegerValue((uint32_t)(m_totalTime * 10000)));
    //echoClientMal.SetAttribute("Interval", TimeValue(Seconds(0.012))); // para quando tiver apenas 6 MPs
    echoClientMal.SetAttribute("Interval", TimeValue(Seconds(0.023)));
    echoClientMal.SetAttribute("PacketSize", UintegerValue(m_packetSize));
    ApplicationContainer clientAppsMal = echoClientMal.Install(nodes.Get(1));
    Ptr<UdpEchoClient> appMal = clientAppsMal.Get(0)->GetObject<UdpEchoClient>();
    appMal->TraceConnectWithoutContext("Tx", MakeCallback(&TxTraceMal));
    appMal->TraceConnectWithoutContext("Rx", MakeCallback(&RxTraceMal));
    clientAppsMal.Start(Seconds(1.1));
    clientAppsMal.Stop(Seconds(m_totalTime + 1.7));
    // ----------------------------- nó 2 malicioso
    // Envio malicioso dos pacotes
    echoClientMal.SetAttribute("Interval", TimeValue(Seconds(0.037)));
    ApplicationContainer clientAppsMal2 = echoClientMal.Install(nodes.Get(4));
    Ptr<UdpEchoClient> appMal2 = clientAppsMal2.Get(0)->GetObject<UdpEchoClient>();
    appMal2->TraceConnectWithoutContext("Tx", MakeCallback(&TxTraceMal));
    appMal2->TraceConnectWithoutContext("Rx", MakeCallback(&RxTraceMal));
    clientAppsMal2.Start(Seconds(1.2));
    clientAppsMal2.Stop(Seconds(m_totalTime + 1.7));
    // ----------------------------- nó 3 malicioso
    // Envio malicioso dos pacotes
    echoClientMal.SetAttribute("Interval", TimeValue(Seconds(0.040)));
    ApplicationContainer clientAppsMal3 = echoClientMal.Install(nodes.Get(6));
    Ptr<UdpEchoClient> appMal3 = clientAppsMal3.Get(0)->GetObject<UdpEchoClient>();
    appMal3->TraceConnectWithoutContext("Tx", MakeCallback(&TxTraceMal));
    appMal3->TraceConnectWithoutContext("Rx", MakeCallback(&RxTraceMal));
    clientAppsMal3.Start(Seconds(1.1));
    clientAppsMal3.Stop(Seconds(m_totalTime + 1.7));
}
...
MeshTest::Run()
{
    ...
    InstallApplication();
    // Agenda o tempo relativo de término do evento
    Simulator::Schedule(Seconds(m_totalTime), &MeshTest::Report, this);
    // Força o Simulator::run voltar para o "caller" quando o tempo de expiração do próximo evento a ser processado
    // é maior ou igual ao StopTime.
    // O tempo de parada é relativo ao tempo de simulação atual
    Simulator::Stop(Seconds(m_totalTime + 2));
    AnimationInterface anim("xml_final/DoS_mesh.xml");
    Simulator::Run();
    Simulator::Destroy();
    std::cout << "UDP echo packets sent: " << g_udpTxCount << " received: " << g_udpRxCount
              << std::endl;
    std::cout << "Mal UDP echo packets sent: " << g_udpTxCountMal << " received: " << g_udpRxCountMal
              << std::endl;
    return 0;
}
